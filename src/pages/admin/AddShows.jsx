    ...
    <>
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button onClick={handleDateTimeAdd} className='bg-red-600/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-red-600 cursor-pointer'>
            Add Time
          </button>
        </div>
      </div>
            ...
            {Object.entries(dateTimeSelection).map(([date, times]) => (
              <li key={date}>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                  {times.map((time) => (
                    <div key={time} className='border border-red-600 px-2 py-1 flex items-center rounded'>
                      <span>{time}</span>
